...
export default async function SubscriptionsPage({ searchParams }: { searchParams: Promise<{ tab?:string; day?:string }> }) {
  ...
  const subscriptions = await prisma.subscription.findMany({ where, include:{ customer:true, product:true, events:{ orderBy:{occurredAt:"desc"}, take:20 } }, orderBy:{ updatedAt:"desc" }, take:100 });
  return <div className="space-y-6"><LiveRefresh/>
    ...
    <div className="panel table-wrap"><table><thead><tr><th>Cliente</th><th>Plano</th><th>Estado atual</th><th>Renovação / próxima cobrança</th><th>Falha / cancelamento</th><th>Último evento</th><th>Ciclos</th></tr></thead><tbody>{subscriptions.map(s=>{const latest=s.events[0],failed=s.events.find(e=>e.type==="REFUSED"||e.type==="LATE"),cancelled=s.canceledAt??s.events.find(e=>e.type==="CANCELED")?.occurredAt;return <tr key={s.id}><td><Link href={`/assinaturas/${s.id}`} className="font-semibold hover:text-[#87f0bf]">{s.customer.name}</Link><span className="block text-xs muted">{s.customer.email}</span></td><td>{s.planName??s.product?.name??"—"}</td><td><StatusBadge status={s.status}/></td><td><span className="block font-medium">Renovou: {dateTime(s.lastRenewedAt)}</span><span className="mt-1 block text-xs muted">Próxima: {dateTime(s.nextChargeAt)}</span></td><td>{failed?<span className="block font-medium text-[#ffcf83]">Tentativa: {dateTime(failed.occurredAt)}</span>:<span className="block muted">Sem falha</span>}{cancelled&&<span className="mt-1 block text-xs text-[#ff9ba8]">Cancelou: {dateTime(cancelled)}</span>}</td><td>{latest?<><StatusBadge status={latest.type}/><span className="mt-1 block text-xs muted">{dateTime(latest.occurredAt)}</span></>:"—"}</td><td>{s.renewalCount} renovações<span className="mt-1 block text-xs muted">{s.cycleCount} ciclos</span></td></tr>})}{!subscriptions.length&&<tr><td colSpan={7} className="py-12 text-center muted">Nenhuma assinatura para este filtro.</td></tr>}</tbody></table></div>
  </div>;
}
